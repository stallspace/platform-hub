...
export default async function VendorDashboardPage() {
  ...
  const STATS = [
    ...
    {
      label: 'Store Visits',
      value: (storeViews ?? 0).toLocaleString(),
      sub: 'All time',
      icon: Eye,
    ...
    {
      label: 'Enquiries',
      value: String(enquiryCount ?? 0),
      sub: unreadEnquiries ? `${unreadEnquiries} unread` : 'None unread',
      icon: MessageSquare,
                ...
                <div key={order.id} className="p-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {order.customer_name} · {new Date(order.created_at).toLocaleDateString('en-ZA')}
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
              ...
              <div key={metric.label}>
                ...
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: metric.max > 0 ? `${(metric.value / metric.max) * 100}%` : '0%' }}
                  />
                </div>
              </div>
            ))}
