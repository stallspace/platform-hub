...
import { LayoutDashboard, Package, Store, MessageSquare, BarChart2, CreditCard, Settings, Crown, LogOut, ShoppingBag, Star, Truck } from 'lucide-react'
...
export default function VendorLayoutClient({ vendor, children }: Props) {
  ...
  const planLabel = vendor.subscription_plan ? vendor.subscription_plan.charAt(0).toUpperCase() + vendor.subscription_plan.slice(1) + ' Plan' : 'No Plan'
      ...
      <aside className="w-64 bg-brand-navy flex-shrink-0 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${active ? 'bg-white/20 text-white font-medium' : 'text-gray-300 hover:bg-white/10 hover:text-white'}`}>
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3 px-3 py-2 mb-1">
            {vendor.logo_url ? (
              ...
              <div className="w-8 h-8 bg-brand-accent rounded-full flex items-center justify-center text-white text-sm font-bold">{initials}</div>
            )}
          ...
          <button onClick={handleSignOut} className="flex items-center gap-2 w-full px-3 py-2 text-gray-400 hover:text-white text-sm transition-colors rounded-lg hover:bg-white/10">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-8 h-8 bg-brand-navy rounded-full flex items-center justify-center text-white text-sm font-bold">{initials}</div>
          </div>
        </header>
