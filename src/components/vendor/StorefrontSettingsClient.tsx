...
import { Upload, Globe, Instagram, Facebook, MessageCircle, ExternalLink, Loader2, Check } from 'lucide-react'
...
export default function StorefrontSettingsClient({ vendor }: Props) {
  ...
  async function uploadImage(file: File, type: string) {
    ...
    const path = vendor.id + '/' + type + '-' + Date.now() + '.' + ext
    const { error: uploadError } = await supabase.storage.from('vendor-assets').upload(path, file, { upsert: true })
  ...
  async function handleSave() {
    ...
    try {
      const { error: updateError } = await supabase.from('vendors').update({
        ...
        social_links: { website: website || undefined, instagram: instagram || undefined, facebook: facebook || undefined, whatsapp: whatsapp || undefined },
      ...
      <div className="space-y-5">
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <div className="p-5 border-b border-gray-50">
            <h2 className="font-semibold text-gray-900">Store Banner</h2>
            <p className="text-xs text-gray-500 mt-0.5">Recommended size: 1200 x 300px</p>
          </div>
          <div className="p-5">
            ...
            <button onClick={() => bannerRef.current?.click()} disabled={uploadingBanner} className="mt-3 text-sm text-gray-500 hover:text-gray-900 flex items-center gap-1.5 disabled:opacity-50">
          ...
          <div className="flex items-center gap-5">
            <div className="w-20 h-20 rounded-2xl border-2 border-dashed border-gray-200 overflow-hidden cursor-pointer hover:border-brand-accent transition-colors flex-shrink-0 relative group" onClick={() => logoRef.current?.click()}>
              {logoUrl
                ...
                : <div className="w-full h-full bg-brand-navy flex items-center justify-center"><span className="text-white text-xl font-bold">{initials}</span></div>
              }
            ...
            <div>
              <button onClick={() => logoRef.current?.click()} disabled={uploadingLogo} className="px-4 py-2 text-sm font-medium border border-gray-200 rounded-lg hover:border-gray-300 hover:bg-gray-50 transition-colors disabled:opacity-50">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          <h2 className="font-semibold text-gray-900 mb-1">About Your Store</h2>
          <p className="text-xs text-gray-500 mb-3">This appears on your public storefront page</p>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <input type="text" value={website} onChange={e => setWebsite(e.target.value)} placeholder="https://yourwebsite.co.za" className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-accent/30 focus:border-brand-accent" />
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <input type="text" value={instagram} onChange={e => setInstagram(e.target.value)} placeholder="username (without @)" className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-accent/30 focus:border-brand-accent" />
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <input type="text" value={facebook} onChange={e => setFacebook(e.target.value)} placeholder="https://facebook.com/yourpage" className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-accent/30 focus:border-brand-accent" />
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center flex-shrink-0"><MessageCircle className="w-4 h-4 text-gray-400" /></div>
              <div className="flex-1">
                <label className="text-xs text-gray-500 mb-1 block">WhatsApp</label>
                <input type="text" value={whatsapp} onChange={e => setWhatsapp(e.target.value)} placeholder="27821234567 (with country code)" className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-accent/30 focus:border-brand-accent" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 pb-6">
          <p className="text-xs text-gray-400">Changes are reflected immediately on your public storefront</p>
